# ArgParser C++ coding style
**The coding style for ArgParser is nearly identical to the style guide used by the [Ladybird project](https://github.com/LadybirdBrowser/ladybird/tree/master).**

This document describes the coding style used for C++ code in the ArgParser project. All new code should conform to this style.

### Names

A combination of CamelCase, snake\_case, and SCREAMING\_CASE:

- Use CamelCase (Capitalize the first letter, including all letters in an acronym) in a class or struct that are public in terms of usage (not used internally)
- Use snake\_case (all lowercase, with underscores separating words) for variable and function names as well as internal classes and structs
...
Do **not** put anything into global scope by declaring "using".

### Types

Omit "int" when using "unsigned" modifier. Do not use "signed" modifier. Use "int" by itself instead or any other fixed length type.
...
Use a private static member function named "get()" to access the instance of the singleton. Expose the private implmentation via a public static function call.
